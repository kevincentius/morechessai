...
public class ChessAI_v9_SemiQuiesce implements ChessAI {
	...
	private double negamax(GameState state, SearchNode node, double depth, double alpha, double beta) throws InterruptedException {
		...
		for (Move move : moves) {
			...
			// in quisce mode, search deeper
			double nextDepth = depth - 1;
			if (depth <= quisceDepth) {
				if (move.getTacticalValue(values) > 0) {
					nextDepth = depth - 0.1;
				}
			}
			double v = -negamax(state, childNode, nextDepth, -beta, -alpha);
