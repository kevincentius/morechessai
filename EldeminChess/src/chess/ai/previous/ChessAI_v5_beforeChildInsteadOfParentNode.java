...
public class ChessAI_v5_beforeChildInsteadOfParentNode {
	...
	private Function<SearchNode_v2, Void> onNewBestNode;
	...
	public void setOnNewBestNode(Function<SearchNode_v2, Void> onNewBestNode) {
		this.onNewBestNode = onNewBestNode;
	}

	public void start(RuleSet ruleSet, GameState gameState, Function<SearchNode_v2, Void> onNewBestNode, long maxTimeMs) {
		this.onNewBestNode = onNewBestNode;
	...
	private Runnable mainLoop = () -> {
		...
		try {
			for (int depth = 1; true; depth++) {
				...
				SearchNode_v2 bestNodeThisDepth = alphaBetaSearch(state, new SearchNode_v2(null, null, 0), depth, -Double.MAX_VALUE, Double.MAX_VALUE);
	...
	private SearchNode_v2 alphaBetaSearch(GameState state, SearchNode_v2 node, double depth, double alpha, double beta) throws InterruptedException {
		...
		for (Move move : legalMoves) {
			...
			SearchNode_v2 childNode = alphaBetaSearch(state, new SearchNode_v2(node, move, 1 - state.getTurn()), nextDepth, -beta, -alpha);
			move.unmake(state, ruleSet);
			if (bestNode == null || childNode.getScore() > bestNode.getScore()) {
				System.out.println(depth + "/" + alpha + "/" + beta + ":" + childNode);
				bestNode = childNode;
