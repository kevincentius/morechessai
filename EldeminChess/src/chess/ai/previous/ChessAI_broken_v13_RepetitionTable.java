...
import chess.ai.RepetitionTable;
...
public class ChessAI_broken_v13_RepetitionTable implements ChessAI {
	
	private double[] values = { 325, 350, 500, 900, 999999, 100, 400 };
	private double contemptFactor = 200; // does not want draw unless down by more than 2 pawns
	...
	private Runnable mainLoop = () -> {
		...
		try {
			...
			for (int depth = 1; true; depth++) {
				...
				rootNode.setScore(negamax(state, rootNode, depth, -Double.MAX_VALUE, Double.MAX_VALUE, 0));
	...
	private double negamax(GameState state, SearchNode node, double depth, double alpha, double beta, int numMoves) throws InterruptedException {
		...
		{
			if (repetitionTable.exists(hash.val1, hash.val2)) {
				return 0;
			}
		...
		repetitionTable.savePosition(hash.val1, hash.val2);
		...
		for (Move move : moves) {
			...
			childNode.setScore(negamax(state, childNode, depth - 1, -beta, -alpha, numMoves + 1));
			double v = -childNode.getScore();
		...
		repetitionTable.removePosition(hash.val1, hash.val2);
